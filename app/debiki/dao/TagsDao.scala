...
object TagsDao {
  ...
  def findTagLabelProblem(tagLabel: TagLabel): Option[ErrorMessageCode] = {
    if (tagLabel.length > MaxTagLength)
      return Some(ErrorMessageCode(s"Tag label too long: '$tagLabel'", "TyE7KPU4R8"))
    if (tagLabel.matches(WhitespaceRegex))
      return Some(ErrorMessageCode(s"Bad tag label, contains whitespace: '$tagLabel'", "TyE4GE8I23"))
    BadTagLabelCharsRegex.findFirstIn(tagLabel) foreach { badChar =>
      return Some(ErrorMessageCode(s"Bad tag label: '$tagLabel', contains char: '$badChar'", "EsE4GE8I2"))
    }
    None
  }
}
