...
case class NotificationGenerator(tx: SiteTransaction, nashorn: Nashorn, config: debiki.Config) {
  ...
  def generateForNewPost(page: Page, newPost: Post, anyNewTextAndHtml: Option[TextAndHtml],
    ...
    //  ... NotificationType.PostTagged, newPost, page.categoryId, ...)
  ...
  private def makeNewPostNotf(notfType: NotificationType, newPost: Post,
    ...
    for {
      ...
      SHOULD // also consider ancestor group notf levels — maybe a group hen is in, has muted the topic?

      NotfLevel.Hushed // Also consider the type of notf: is it a direct message? Then send
  ...
  private def makeNewPostSubscrNotfFor(notfPrefs: Seq[PageNotfPref], newPost: Post,
    ...
    val membersWithPrefsConfigdById = tx.loadUsersAsMap(notfPrefs.map(_.peopleId))
    ...
    for {
      ...
      member <- membersWithPrefsConfigdById.get(notfPref.peopleId)
    } {
    ...
    for {
      notfPref: PageNotfPref <- notfPrefs
      maybeGroup <- membersWithPrefsConfigdById.get(notfPref.peopleId)
    ...
    def maybeMakeNotfs(member: User, notfPref: PageNotfPref) {
      if (memberIdsHandled.contains(member.id))
        return
      memberIdsHandled += member.id
      // Don't do this earlier, because if less-than, then still want to insert into memberIdsHandled.
